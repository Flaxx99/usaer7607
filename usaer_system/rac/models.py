...
SUBCLASIFICACION_CHOICES = DISCAPACIDAD_SUB + DIFICULTADES_SUB + TRASTORNOS_SUB + APTITUDES_SUB
...
class RegistroRAC(models.Model):
    ...
    zona_regular = models.CharField("Zona Escuela Regular", max_length=10, null=True, blank=True)

    # Datos del alumno (se autorrellenan)
    curp = models.CharField("CURP Alumno", max_length=18, null=True, blank=True)
    ...
    edad = models.PositiveSmallIntegerField("Edad Alumno", null=True, blank=True)
    grado = models.CharField("Grado Alumno", max_length=20, null=True, blank=True)

    # Servicio y supervisión
    service_type = models.CharField(
        "Tipo de servicio", max_length=20, choices=SERVICE_CHOICES, default="USAER"
    )
    sup_especial_cct = models.CharField("CCT Supervisión", max_length=20, blank=True, null=True)
    sup_especial_zona = models.CharField("Zona Supervisión", max_length=10, blank=True, null=True)

    # Centro de Educación Especial (se llenan dinámicamente o vía config)
    centro_cct = models.CharField(
        "CCT Centro Educación Especial", max_length=20, blank=True, null=True
    )
    centro_nombre = models.CharField(
        "Nombre Centro Educación Especial", max_length=200, blank=True, null=True
    )
    ...
    escuela_basica = models.ForeignKey(
        Escuela, on_delete=models.PROTECT, related_name="rac_basica", verbose_name="Escuela básica"
    )

    # Clasificación y subclasificación
    clasificacion = models.CharField(
        "Clasificación", max_length=30, choices=Alumno.CLASIFICACION_CHOICES
    )
    subclasificacion = models.CharField(
        "Subclasificación", max_length=20, choices=SUBCLASIFICACION_CHOICES
    )
