...
class RegistroRACForm(forms.ModelForm):
    ...
    def __init__(self, *args, **kwargs):
        ...
        for fld in ["zona_regular", "curp", "edad", "grado"]:
            self.fields[fld].widget.attrs.update({"readonly": True, "class": "form-control"})
        self.fields["sexo"].widget.attrs.update({"disabled": True, "class": "form-select"})
        for fld in ["sup_especial_cct", "sup_especial_zona", "centro_cct", "centro_nombre"]:
            self.fields[fld].widget.attrs.update({"readonly": True, "class": "form-control"})
        ...
        # ID y clases para JS
        self.fields["alumno"].widget.attrs.update({"id": "id_alumno", "class": "form-select"})
        ...
        self.helper.layout = Layout(
            ...
            Div(Field("clasificacion"), Field("subclasificacion"), css_class="row g-3"),
            Field("observaciones"),
        ...
        self.fields["subclasificacion"].choices = [
    ...
    def clean(self):
        ...
        if (
            alumno
            and RegistroRAC.objects.filter(alumno=alumno, fecha_registro=date.today()).exists()
        ):
            raise ValidationError("Este alumno ya tiene un registro RAC para la fecha de hoy.")
        return cleaned
