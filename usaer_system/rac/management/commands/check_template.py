...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        template_path = (
            settings.BASE_DIR / "rac" / "static" / "excel_templates" / "rac_template.xlsx"
        )
        ...
        try:
            ...
            if sheet_name in wb.sheetnames:
                ...
                self.stdout.write(
                    self.style.WARNING(f"--- Analizando formato de la celda {cell_to_check} ---")
                )

                self.stdout.write(f"Tiene estilo: {cell.has_style}")
                self.stdout.write(
                    f"  - Fuente: {cell.font.name}, Tamaño: {cell.font.sz}, Negrita: {cell.font.b}, Color: {cell.font.color}"
                )
                self.stdout.write("  - Bordes:")
                ...
                self.stdout.write(
                    f"  - Relleno: {cell.fill.patternType}, Color de Fondo: {cell.fill.fgColor}, Color de Patrón: {cell.fill.bgColor}"
                )
                self.stdout.write(
                    f"  - Alineación: Horizontal={cell.alignment.horizontal}, Vertical={cell.alignment.vertical}"
                )

            else:
                self.stdout.write(
                    self.style.ERROR(
                        f"Error: La hoja llamada '{sheet_name}' no se encuentra en la plantilla."
                    )
                )
            ...
            self.stdout.write(
                self.style.ERROR(
                    "Error: No se encontró el archivo de la plantilla en la ruta especificada."
                )
            )
