...
class SystemConfiguration(models.Model):
    centro_nombre = models.CharField(
        _("Nombre del Centro (USAER)"), max_length=200, default="USAER 7607"
    )
    centro_cct = models.CharField(_("CCT del Centro"), max_length=20, default="08FUA0093E")
    director_responsable = models.CharField(
        _("Nombre del Director Responsable"), max_length=200, default="Nubia Idaly Solis Mendias"
    )
    sup_especial_cct = models.CharField(
        _("CCT Supervisión Especial"), max_length=20, default="08FUA0041G"
    )
    sup_especial_zona = models.CharField(
        _("Zona Supervisión Especial"), max_length=10, default="22"
    )
    ubicacion_centro = models.CharField(
        _("Ubicación del Centro (Ciudad)"), max_length=200, default="Juan Aldama, Chihuahua"
    )
...
class User(AbstractUser):
    ...
    email = models.EmailField(
        _("Correo institucional"), unique=True, help_text=_("Correo institucional del usuario")
    )
    ...
    curp_regex = RegexValidator(
        ...
        message=_(
            "Formato de CURP inválido. Asegúrate de que tenga 18 caracteres y siga el formato oficial."
        ),
    )
    ...
    role = models.CharField(
        _("Función"), max_length=30, choices=Role.choices, default=Role.MAESTRO_APOYO, db_index=True
    )
    ...
    telefono = models.CharField(
        _("Teléfono particular"), max_length=15, validators=[phone_regex], blank=True
    )
    ...
    rfc = models.CharField(
        _("R.F.C."), max_length=13, validators=[rfc_regex], blank=True, null=True, unique=True
    )
    curp = models.CharField(
        _("C.U.R.P."), max_length=18, validators=[curp_regex], blank=True, null=True, unique=True
    )
    ...
    nivel = models.CharField(
        _("Escolaridad"), max_length=4, choices=NivelEducativo.choices, blank=True
    )
    ...
    situacion = models.CharField(
        _("Situación laboral"), max_length=5, choices=Situacion.choices, blank=True
    )
    ...
    class Meta:
        ...
        constraints = [
            ...
            models.UniqueConstraint(
                fields=["curp"], name="unique_curp", condition=models.Q(curp__isnull=False)
            ),
            models.UniqueConstraint(
                fields=["rfc"], name="unique_rfc", condition=models.Q(rfc__isnull=False)
            ),
        ]
        indexes = [
...
class CalendarEvent(models.Model):
    ...
    event_type = models.CharField(
        _("Tipo de evento"), max_length=20, choices=EventType.choices, default=EventType.OTRO
    )
    status = models.CharField(
        _("Estado"), max_length=20, choices=EventStatus.choices, default=EventStatus.PENDIENTE
    )
    priority = models.CharField(
        _("Prioridad"), max_length=20, choices=Priority.choices, default=Priority.MEDIA
    )

    created_by = models.ForeignKey(
        User, on_delete=models.CASCADE, related_name="events_created", verbose_name=_("Creado por")
    )
    assigned_to = models.ForeignKey(
        User, on_delete=models.CASCADE, related_name="events_assigned", verbose_name=_("Asignado a")
    )
