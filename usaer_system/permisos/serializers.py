...
class PermisoSerializer(serializers.ModelSerializer):
    ...
    def validate(self, data):
        ...
        fecha_inicio = (
            data.get("fecha_inicio") or self.instance.fecha_inicio
            if self.instance
            else data.get("fecha_inicio")
        )
        fecha_fin = (
            data.get("fecha_fin") or self.instance.fecha_fin
            if self.instance
            else data.get("fecha_fin")
        )
        ...
        if fecha_inicio and fecha_fin:
            ...
            if (fecha_fin - fecha_inicio).days > 30:
                raise serializers.ValidationError(
                    "No se pueden solicitar más de 30 días de permiso consecutivos."
                )
