...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        for exp in Expediente.objects.all():
            carpeta = os.path.join(settings.MEDIA_ROOT, f"expedientes/alumno_{exp.alumno.id}")
            if not os.path.isdir(carpeta):
                continue

            for nombre in os.listdir(carpeta):
                if (
                    nombre.startswith("otros.")
                    and not exp.otros_archivos.filter(archivo__endswith=nombre).exists()
                ):
