...
class ExpedienteForm(forms.ModelForm):
    class Meta:
        ...
        widgets = {
            "informe_deteccion": forms.ClearableFileInput(
                attrs={"class": "form-control", "accept": ".pdf,.doc,.docx,.xlsx"}
            ),
            "informe_psicopedagogico": forms.ClearableFileInput(
                attrs={"class": "form-control", "accept": ".pdf,.doc,.docx,.xlsx"}
            ),
            "plan_intervencion": forms.ClearableFileInput(
                attrs={"class": "form-control", "accept": ".pdf,.doc,.docx,.xlsx"}
            ),
    ...
    def __init__(self, *args, user=None, **kwargs):
        ...
        if self.instance and self.instance.pk:
            ...
            if user and (
                user.is_superuser
                or user.role in [User.Role.ADMINISTRADOR.value, User.Role.SECRETARIO.value]
            ):
                ...
                self.fields["alumno"].queryset = Alumno.objects.filter(
                    profesor=user, expediente__isnull=True
                )
            else:
