...
class PresenteFilter(admin.SimpleListFilter):
    ...
    def lookups(self, request, model_admin):
        return [(True, _("Presente")), (False, _("Ausente"))]
...
class AsistenciaAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        (_("Información General"), {"fields": ("profesor", "escuela", "fecha", "presente")}),
        (_("Horarios"), {"fields": ("hora_entrada", "hora_salida")}),
        (_("Registro"), {"fields": ("created_at",)}),
    )
    ...
    def profesor_link(self, obj):
        if obj.profesor:
            return format_html(
                '<a href="{}">{}</a>',
                reverse("admin:usuarios_user_change", args=[obj.profesor.id]),
                obj.profesor.get_full_name(),
            )
    ...
    def escuela_link(self, obj):
        return format_html(
            '<a href="{}">{}</a>',
            reverse("admin:escuelas_escuela_change", args=[obj.escuela.id]),
            obj.escuela.nombre,
        )
