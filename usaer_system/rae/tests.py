...
class RAEModelTest(APITestCase):
    def setUp(self):
        ...
        self.escuela = Escuela.objects.create(
            clave_estatal="E7", cct="CCT7", nombre="Escuela RAE", nivel="Primaria",
            domicilio="Dir", colonia="Col", zona="Z7"
        )
        self.maestro = User.objects.create_user(
            email="maestro_rae@example.com", numero_empleado="EMP009", password="pass",
...
class RAEViewsTest(APITestCase):
    def setUp(self):
        ...
        self.escuela = Escuela.objects.create(
            clave_estatal="E8", cct="CCT8", nombre="Escuela RAE Views", nivel="Primaria",
            domicilio="Dir", colonia="Col", zona="Z8"
        )
        self.maestro = User.objects.create_user(
            email="maestro_rae_view@example.com", numero_empleado="EMP010", password="pass",
            escuela=self.escuela, role=getattr(User, 'Role', None) and getattr(User.Role, 'MAESTRO_APOYO', 'MAESTRO_APOYO')
        )
        self.admin = User.objects.create_superuser(
            email="admin_rae_view@example.com", numero_empleado="ADM006", password="pass",
            escuela=self.escuela
        )
        self.alumno = Alumno.objects.create(
            profesor=self.maestro,
    ...
    def test_crear_registro_rae(self):
        ...
        escuela_nueva = Escuela.objects.create(
            clave_estatal="E9", cct="CCT9", nombre="Escuela RAE Nueva", nivel="Primaria",
            domicilio="Dir", colonia="Col", zona="Z9"
        )
        form_data = {
        ...
        self.assertTrue(RegistroRAE.objects.filter(escuela=self.escuela, ciclo_escolar=self.ciclo).exists())
    ...
    def test_editar_registro_rae(self):
        ...
        response = self.client.patch(reverse("rae:registros-detail", args=[self.registro.pk]), data=form_data)
...
class RAEExportTest(APITestCase):
    ...
    def setUp(self):
        ...
        self.rae1 = RegistroRAE.objects.create(escuela=self.school1, ciclo_escolar=self.ciclo, creado_por=self.teacher1, docente_hombres=1, docente_mujeres=1)
        self.rae2 = RegistroRAE.objects.create(escuela=self.school2, ciclo_escolar=self.ciclo, creado_por=self.teacher2, docente_hombres=1, docente_mujeres=1)
        
        RAEAlumno.objects.create(registro=self.rae1, alumno=self.student1_t1, capturado_por=self.teacher1)
        RAEAlumno.objects.create(registro=self.rae1, alumno=self.student2_t1, capturado_por=self.teacher1)
        RAEAlumno.objects.create(registro=self.rae2, alumno=self.student1_t2, capturado_por=self.teacher2)

    def test_export_rae_excel_view_for_teacher(self):
        self.client.force_authenticate(self.teacher1)
        # Use a valid PK
        response = self.client.get(reverse('rae:exportar_rae_excel', args=[self.rae1.pk]), HTTP_IS_TEST='True')
        self.assertEqual(response.status_code, 200)
