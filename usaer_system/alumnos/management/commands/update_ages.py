    ...
    def handle(self, *args, **options):
        ...
        for alumno in alumnos_to_update:
            ...
            try:
                alumno.save()
                updated_count += 1
                self.stdout.write(self.style.SUCCESS(f'Successfully updated age for: {alumno.get_full_name()}'))
            except Exception as e:
                self.stderr.write(self.style.ERROR(f'Could not update age for {alumno.get_full_name()}: {e}'))
        ...
        if skipped_count > 0:
            self.stdout.write(self.style.WARNING(f"{skipped_count} students were skipped because they don't have a date of birth set."))
