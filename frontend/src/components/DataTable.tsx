    ...
    return (
        <div className="space-y-4" aria-live="polite">
            ...
            <div className="card bg-base-100 shadow-sm border border-base-300 overflow-hidden">
                 {/* ——— MOBILE CARD VIEW ——— */}
                 <div className="sm:hidden divide-y divide-base-200" role="table">
                     {table.getRowModel().rows.length > 0 ? (
                         table.getRowModel().rows.map(row => (
                             <div key={row.id} className="p-4 space-y-2" role="row">
                                 {row.getVisibleCells().map(cell => {
                                     const header = cell.column.columnDef.header;
                                     const headerText = typeof header === 'string' ? header : '';
                                     return (
                                         <div key={cell.id} className="flex items-start gap-2 text-sm" role="cell">
                                             {headerText && (
